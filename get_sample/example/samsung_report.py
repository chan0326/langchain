...
class SamsungReport:
    def __init__(self) -> None:
        ...
        self.okt = Okt()
        self.data = DataModel()
        self.data.dname = 'C:\\Users\\AIA\\python\\vienna-chat-server\\get_sample\\example\\data\\'
    ...
    def stopword_embedding(self):
        ...
        for token in tokens:
            ...
            _ = [j[0] for j in pos if j[1] == 'Noun'] # _ : 함수안에서만 사용하는 변수 선언   // j[0] : 단어, j[1] : 품사
            if len(''.join(_)) > 1:
    ...
    def draw_wordcloud(self):
        _ = self.morphemes
        freqtxt = pd.Series(dict(FreqDist(_))).sort_values(ascending=False) # 내림차순 정렬
        print(freqtxt[:10])
        wcloud = WordCloud(f'{self.data.dname}D2Coding.ttf', relative_scaling=0.2 , background_color='white').generate_from_frequencies(freqtxt)
        plt.figure(figsize=(12,12)) # 그림 사이즈
        plt.imshow(wcloud, interpolation='bilinear') # 이미지의 부드럽기 정도
        plt.axis('off') # 축 제거
        plt.show() # 그림 출력
...
하지만 이러한 punkt모델에도 치명적인 단점이 있는데,
모든 약어를 학습하지 못했다보니, Vol. 13, Apr. 13 과 같은 표현 및 U.S. Pat. No. 134 과 같은 복잡한 약어는
